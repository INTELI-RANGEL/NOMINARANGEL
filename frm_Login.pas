...
begin
  Try
    ...
    NameDataset := cbBase.Text; //Globales.Elemento('DATABASE').AsString;
    ...
    Try
      ...
      if Not proxy.InitGlobal(NameDataset, 'ALM_USUARIO', cbEditUsr.Text + ',' + EditPass.Text, NameDataSet) then
      ...
      Globales.SetValue('Password', EditPass.Text);
      Globales.SetValue('DATABASE', cbBase.Text);
    Finally
      Proxy.Free;
    End;
    ...
    {if Not CrearConjunto(ClientModule1.cdPermisos, 'cmt_permisos', ccSelect) then
      raise InteligentException.Create('No se encuentran los permisos de Usuario');

    if Not CargarDatosFiltrados(ClientModule1.cdPermisos, 'sIdUsuario', [Globales.Elemento('Usuario').AsString]) then
      raise InteligentException.Create('No se pueden filtrar los permisos de Usuario');

    ClientModule1.cdPermisos.Open;}
