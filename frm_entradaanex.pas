...
uses
  ...
  cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, cxGroupBox, cxSplitter;
...
type
    ...
    anexo_psuministrosNumeroActividad: TWideStringField;
    anexo_psuministrodCantidad: TFloatField;
    anexo_psuministroiItemOrden: TWideStringField;
    anexo_psuministromDescripcion: TMemoField;
    anexo_psuministrosMedida: TWideStringField;
    ...
    ReportesIdTipo: TWideStringField;
    ReportesReferencia: TWideStringField;
    ReportesIdProveedor: TWideStringField;
    ReportesOrigen: TWideStringField;
    ...
    anexo_psuministrosWbs: TWideStringField;
    PopupMenu1: TPopupMenu;
    CargarMateriales1: TMenuItem;
    zq_ParInsiId: TIntegerField;
    zq_ParInssContrato: TWideStringField;
    zq_ParInsiFolioSuministro: TIntegerField;
    zq_ParInssWbs: TWideStringField;
    zq_ParInssNumeroActividad: TWideStringField;
    zq_ParInssNumeroOrden: TWideStringField;
    zq_ParInsdFecha: TDateField;
    zq_ParInssIdInsumo: TWideStringField;
    zq_ParInsmDescripcion: TMemoField;
    zq_ParInssMedida: TWideStringField;
    zq_ParInsdCantidad: TFloatField;
    ReportesWbs: TWideStringField;
    ...
    procedure LCmbPartidaKeyPress(Sender: TObject; var Key: Char);
    procedure LCmbPartidaEnter(Sender: TObject);
    procedure LCmbPartidaExit(Sender: TObject);
    procedure mniMaterialesClick(Sender: TObject);
...
begin
    ...
    Begin
      LCmbPartida.Text:=anexo_psuministro.FieldValues['sNumeroActividad'] ;
     //   tsNumeroActividad.Text := anexo_psuministro.FieldValues['sNumeroActividad'] ;
        ...
        Connection.qryBusca.Params.ParamByName('actividad').Value := LCmbPartida.Text ;
        ...
        QrySuministro.Params.ParamByName('Actividad').Value := LCmbPartida.Text ;
...
begin
    global_partida := LCmbPartida.Text ;
...
begin
    ...
    LCmbPartida.Properties.ReadOnly := True ;
    ...
    QrPartidas.Active:=False;
    QrPartidas.ParamByName('contrato').AsString:=global_contrato;
    QrPartidas.ParamByName('convenio').AsString:=global_convenio;
    QrPartidas.Open;
...
begin
    ...
    If lValido Then
        ...
        Begin
            ...
            LCmbPartida.Properties.ReadOnly := False ;
...
begin
  ...
  cadenas.Add(LCmbPartida.Text);cadenas.Add(tsMedida.Text);cadenas.Add(tdCantidadAnexo.Text);
  ...
  If LCmbPartida.Text <> '' then
    If Not lExisteActividad ( LCmbPartida.Text ) then
        LCmbPartida.SetFocus
    Else
    Begin
        try
            ...
            Begin
                ...
                connection.zCommand.Params.ParamByName('Folio').value := anexo_suministro.FieldValues['iFolio'] ;
                connection.zCommand.Params.ParamByName('Actividad').DataType := ftString ;
                connection.zCommand.Params.ParamByName('Actividad').value := LCmbPartida.Text ;
...
procedure Tfrmentradaanex.LCmbPartidaEnter(Sender: TObject);
begin
    imgNotas.Visible := False ;
  LCmbPartida.Style.Color := global_color_entradaERP;
end;

procedure Tfrmentradaanex.LCmbPartidaExit(Sender: TObject);
begin
LCmbPartida.Style.Color := global_color_salidaERP;
  If LCmbPartida.Properties.ReadOnly = False Then
  Begin
      tmDescripcion.Text := '' ;
      tdCantidadAnexo.Value := 0 ;
      tsMedida.Text := '' ;
...
procedure Tfrmentradaanex.LCmbPartidaKeyPress(Sender: TObject; var Key: Char);
begin
      If Key = #13 Then
        tdCantidad.SetFocus 
end;
...
procedure Tfrmentradaanex.PgControlChanging(Sender: TObject;
...
procedure Tfrmentradaanex.PopupPrincipalPopup(Sender: TObject);
begin
  mniMateriales.Enabled:=False;
  if anexo_psuministro.RecordCount>0 then
    mniMateriales.Enabled:=True;
end;
...
procedure Tfrmentradaanex.mniMaterialesClick(Sender: TObject);
begin
  zq_InsxPar.Active := False;
  If Connection.configuracion.fieldValues['sAlmcon'] = 'CONTRATOS' Then
    zq_InsxPar.ParamByName('iContrato').AsString := global_contrato
  else
    zq_InsxPar.ParamByName('iContrato').AsInteger := -1;
