...
uses
  ...
  cxGrid, CxGridExportLink, ShellApi, cxContainer, cxTextEdit, cxHyperLinkEdit,
  JvLinkLabel, NxCollection;
...
type
  TfrmUsuarios = class(TForm)
    ...
    procedure LnkLblEmailClick(Sender: TObject);
...
uses frm_ConfiguraMail, Utilerias, UInteliDialog, Frm_SetupMail;
...
begin
  ...
  action := cafree ;
        // Para localizar el usuario y determinar si puede Insertar, Eliminar, Editar, Imprimir
            ...
            Connection.QryBusca.SQL.Text := 'select sMail, eInsertar, eEditar, eEliminar, eImprimir, eGrabar from usuarios ' +
...
procedure TfrmUsuarios.LnkLblEmailClick(Sender: TObject);
begin
  Application.CreateForm(TFrmSetupMail,FrmSetupMail);
  try
    FrmSetupMail.sIdUser:=Usuarios.FieldByName('sIdUsuario').AsString;
    FrmSetupMail.ShowModal;
  finally
    FrmSetupMail.Destroy;
  end;
end;
