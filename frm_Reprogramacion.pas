...
uses
  ...
  cxGridDBTableView, cxGridLevel, cxClasses, cxGridCustomView, cxGrid, JvExMask,
...
uses frm_connection, frm_admonCatalogos;

{$R *.dfm}

procedure TfrmReprogramacion.FormActivate(Sender: TObject);
begin
  if TiposdeConvenio.Active then
    TiposdeConvenio.Refresh;
end;
...
procedure TfrmReprogramacion.btn1Click(Sender: TObject);
begin
  HojaActiva:='TabTipReprogramacion';
  if not MostrarVentana('frmAdmonCatalogos') then
  begin
