...
type
  TfrmIndirectos = class(TForm)
    ...
    Label7: TLabel;
    Label8: TLabel;
    tdCostoCampoMN: TDBEdit;
    tdCostoCampoDLL: TDBEdit;
    Label10: TLabel;
    tdPorcentajeAdm: TDBEdit;
    Label11: TLabel;
    tdPorcentajeCampo: TDBEdit;
    tsAgrupacion: TDBLookupComboBox;
    Label12: TLabel;
    GruposIndirecto: TZReadOnlyQuery;
    ds_gruposindirecto: TDataSource;
    AnexoH41: TMenuItem;
    ds_indirectos: TfrxDBDataset;
    AnexoIndirectos: TZReadOnlyQuery;
    AnexoIndirectossContrato: TStringField;
    AnexoIndirectossIdIndirecto: TStringField;
    AnexoIndirectossDescripcion: TStringField;
    AnexoIndirectossMedida: TStringField;
    AnexoIndirectosdCantidad: TFloatField;
    AnexoIndirectosdCostoMN: TFloatField;
    AnexoIndirectosdCostoDLL: TFloatField;
    AnexoIndirectosdVentaMN: TFloatField;
    AnexoIndirectosdVentaDLL: TFloatField;
    AnexoIndirectosdFechaInicio: TDateField;
    AnexoIndirectosdFechaFinal: TDateField;
    AnexoIndirectosdPorcientoCampo: TFloatField;
    AnexoIndirectosdPorcientoAdmon: TFloatField;
    AnexoIndirectosdCostoMNCampo: TFloatField;
    AnexoIndirectosdCostoDLLCampo: TFloatField;
    AnexoIndirectosidGrupo: TIntegerField;
    AnexoIndirectosgrupo: TStringField;
    AnexoIndirectostotal: TLargeintField;
    ...
    procedure AnexoH41Click(Sender: TObject);
...
procedure TfrmIndirectos.AnexoH41Click(Sender: TObject);
begin
   AnexoIndirectos.Active := False;
   AnexoIndirectos.SQL.Clear;
   AnexoIndirectos.SQL.Add('select i.*, gi.sDescripcion as grupo, '+
                    '(select COUNT(scontrato) from indirecto where idGrupo = i.idGrupo group by idGrupo) as total '+
                    'from indirecto i '+
                    'left join grupos_indirecto gi on (gi.idGrupo = i.idGrupo) '+
                    'where sContrato =:Contrato '+
                    'group by i.idGrupo, i.sIdIndirecto');
   AnexoIndirectos.ParamByName('Contrato').AsString := global_contrato;
   AnexoIndirectos.Open;

   if AnexoIndirectos.RecordCount > 0 then
   begin
      ...
      frxAnexoDT4.LoadFromFile(Global_Files+ global_miReporte + '_indirectos.fr3') ;
      if not FileExists(global_files + global_miReporte + '_indirectos.fr3') Then
         showmessage('El archivo de reporte ' + global_files + global_Mireporte+'_indirectos.fr3 no existe, notifique al administrador del sistema')
      else
...
procedure TfrmIndirectos.btnPrinterClick(Sender: TObject);
begin
   AnexoIndirectos.Active := False;
   AnexoIndirectos.SQL.Clear;
   AnexoIndirectos.SQL.Add('select i.*, gi.sDescripcion as grupo, '+
...
begin
  ...
  gruposIndirecto.Active:=False;
  gruposIndirecto.Open ;
