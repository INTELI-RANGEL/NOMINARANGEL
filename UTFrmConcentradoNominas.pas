...
const
  TitFields: Array[0..16] of String = ('Dias', 'Sueldo', 'T.Extra', 'Vacaciones', 'Pma. Vac.', 'Aguinaldo', 'Indemn.', 'Subsidio', 'ISPT A.S.', 'ISPT', 'IMSS', 'Sindicato', 'Infonavit', 'Pen. Alim.', 'Percep.', 'Deduc.', 'Neto');
  ListaFields: Array[0..16] of String = ('DiasTrabajados', 'Sueldo', 'TiempoExtra', 'Vacaciones', 'PrimaVacacional', 'Aguinaldo', 'Indemnizacion', 'Subsidio', 'ISPTAS', 'ISPT', 'IMSSEmpleado', 'Sindicato', 'PrestamoInfonavit', 'PensionAlimenticia', 'TotPer', 'TotDed', 'Neto');
...
procedure TFrmConcentradoNominas.btnCancelarClick(Sender: TObject);
begin
  Close;
end;
    ...
    try
      ...
      try
        ...
        begin
          ...
          begin
            Titulos.Add(TitFields[i]);
            Valores.Add(ListaFields[i]);
            Inc(Cuantas);
          end;
        ...
        // Obtener las fechas de inicio y la de termino
        Inicio := cdConcentrado.FieldByName('FechaNomina').AsDateTime;
        Termino := cdConcentrado.FieldByName('FechaTerminoNomina').AsDateTime;
        while not cdConcentrado.Eof do
        begin
          if Inicio > cdConcentrado.FieldByName('FechaNomina').AsDateTime then
            Inicio := cdConcentrado.FieldByName('FechaNomina').AsDateTime;

          if Termino < cdConcentrado.FieldByName('FechaTerminoNomina').AsDateTime then
            Termino := cdConcentrado.FieldByName('FechaTerminoNomina').AsDateTime;
...
begin
  ...
  if VarName = 'Inicio' then
    Value := Inicio;

  if VarName = 'Termino' then
    Value := Termino;
  ...
  if Copy(VarName, 1, 5) = 'Valor' then
  begin
    try
      Numero := StrToInt(Copy(VarName, 6, 2));
