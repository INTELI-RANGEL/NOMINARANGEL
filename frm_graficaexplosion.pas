...
uses
  ...
  ZAbstractDataset, ZDataset,UnitTBotonesPermisos,UFunctionsGHH,
  global, frm_barra, db, Grids, frm_Connection, StdCtrls,
  DBCtrls, ComCtrls, Mask, Utilerias, StrUtils, Menus, Buttons, rxCurrEdit, rxToolEdit, MasUtilerias,
...
begin
  band:=False;
    if not FileExists(global_files + 'GraficaExplosion.fr3') then
       showmessage('El archivo de reporte ' + 'GraficaExplosion.fr3 no existe, notifique al administrador del sistema')
   else
     begin
      frxReport1.LoadFromFile(global_files + 'GraficaExplosion.fr3');
