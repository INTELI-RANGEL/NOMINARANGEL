...
procedure TfrmActividadesAnexo2.ActividadesxAnexoAfterScroll(DataSet: TDataSet);
begin
  if not ActividadesxAnexo.ControlsDisabled then
  begin
    roqAnexo.RecNo := ActividadesxAnexo.RecNo;
    NxPCAnexoChange(self);
    Grid_Actividades.Repaint;

    if roqAnexo.RecordCount > 0 then
      Grid_Actividades.Hint := roqAnexo.FieldValues['mDescripcion'];

    if roqAnexo.RecordCount > 0 then
    begin
      if (ActividadesxAnexo.State <> dsInsert) and (ActividadesxAnexo.State <> dsEdit) then
      begin
        if (roqAnexo.FieldValues['sTipoActividad'] = 'Paquete') then
          sPaquete := roqAnexo.FieldValues['sWBS']
        else
          sPaquete := roqAnexo.FieldValues['sWBSAnterior'];
        ...
        if not banderaAlcance then begin
