...
uses
  ...
  EditCalc, NxEdit, sMaskEdit, sCustomComboEdit, sCurrEdit, masUtilerias,
...
type
  TfrmBitacoraxAlcance = class(TForm)
   Label1: TLabel;
    ...
    tdCantidad: TJvCalcEdit;
    BitacoradeAlcancessNumeroOrden: TStringField;
    AlcancessTipoActividad: TStringField;
    Label6: TLabel;
...
begin
  ...
  begin
    ...
    qryBusca.Params.ParamByName('Contrato').Value := param_global_Contrato;
    ...
    qryBusca.Params.ParamByName('Convenio').Value := convenio_reporte;
    qryBusca.Open;
...
begin
  try
    ...
    Connection.CommandTrx.Params.ParamByName('contrato').value := param_global_contrato;
...
begin
  ...
  begin
    ...
    BitacoradeAlcances.Params.ParamByName('contrato').Value := param_global_contrato;
...
begin
  try
    ...
    if OrdenesdeTrabajo.RecordCount > 1 then
      tdAvanceGlobal.Value := cfnCalculaAvances(param_global_contrato, tsNumeroOrden.Text, 'NIL', global_turno_reporte, True, tdIdFecha.Date, 'Avanzada', frmBitacoraxAlcance)
    else
      tdAvanceGlobal.Value := cfnCalculaAvances(param_global_contrato, tsNumeroOrden.Text, 'NIL', global_turno_reporte, False, tdIdFecha.Date, 'Avanzada', frmBitacoraxAlcance);
    BotonPermiso.Free;
  except
...
var
  ...
  iPosicion: Integer;
  ListaRef, ListaCant, ListaFolio, sTipo: string;
  nombres, cadenas: TStringList;
begin
  {Validacion de campos}
  nombres := TStringList.Create; cadenas := TStringList.Create;
  nombres.Add('No. Partida'); nombres.Add('Fase');
  cadenas.Add(tsNumeroActividad.Text); cadenas.Add(tiFase.Text);
    ...
    if OpcButton = 'New' then
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Connection.qryBusca.Active := False;
                Connection.qryBusca.SQL.Clear;
                Connection.qryBusca.SQL.Add('SELECT a.dAvance, a.sTipoActividad FROM alcancesxactividad a inner join actividadesxorden o on (' +
                ...
                begin
                    ...
                    sTipo          := Connection.qryBusca.FieldValues['sTipoActividad'];
                end;
                ...
                AvanceMaximo.SQL.Add('SELECT Sum(dAvance) as Avance FROM bitacoradeactividades where ' +
                  ...
                  'sWbs = :wbs and sNumeroActividad = :Actividad and sIdTipoMovimiento = "A" and lAlcance = "No" ' +
                  'Group By sContrato');
                ...
                AvanceMaximo.SQL.Add('SELECT Sum(dAvance) as Avance FROM bitacoradeactividades where ' +
                  ...
                  'sNumeroOrden = :orden and sWbs = :wbs and sNumeroActividad = :Actividad and sIdTipoMovimiento = "A" and lAlcance = "No" ' +
                  'Group By sContrato');
                AvanceMaximo.Params.ParamByName('Contrato').DataType := ftString;
                AvanceMaximo.Params.ParamByName('Contrato').Value := param_Global_Contrato;
                AvanceMaximo.Params.ParamByName('Fecha').DataType := ftDate;
              ...
              try
                ...
                connection.zCommand.SQL.Add('INSERT INTO bitacoradeactividades ( sContrato , dIdFecha , iIdDiario, sIdTurno, ' +
                  ' sNumeroOrden, sWbs, sNumeroActividad, sIdTipoMovimiento, dAvance ,mDescripcion, mNotas, lAlcance ) ' +
                  ' VALUES (:contrato, :fecha, :diario, :turno, :orden, :wbs, :actividad, :tipo, :Avance, :descripcion, :Notas, :alcance)');
                connection.zCommand.Params.ParamByName('contrato').DataType    := ftString;
                connection.zCommand.Params.ParamByName('contrato').value       := param_Global_Contrato;
                connection.zCommand.Params.ParamByName('fecha').DataType       := ftDate;
                connection.zCommand.Params.ParamByName('fecha').value          := tdIdFecha.Date;
                ...
                connection.zCommand.Params.ParamByName('alcance').DataType     := ftString;
                if sTipo = 'Paquete' then
                   connection.zCommand.Params.ParamByName('alcance').value     := 'Si'
                else
                   connection.zCommand.Params.ParamByName('alcance').value     := 'No';
                connection.zCommand.ExecSQL;
        ...
        begin
            ...
            connection.zCommand.Active := False;
            connection.zCommand.SQL.Clear;
            connection.zCommand.SQL.Add('select * from bitacoradeactividades where sContrato =:Contrato and dIdFecha =:Fecha and sIdTurno =:Turno and sNumeroOrden =:Orden and sWbs =:Wbs and sNumeroActividad =:Actividad and sIdTipoMovimiento =:Tipo ');
            connection.zCommand.Params.ParamByName('contrato').DataType  := ftString;
            connection.zCommand.Params.ParamByName('contrato').value     := param_Global_Contrato;
            connection.zCommand.Params.ParamByName('fecha').DataType     := ftDate;
            connection.zCommand.Params.ParamByName('fecha').value        := tdIdFecha.Date;
...
begin
  ...
  begin
    ...
    begin
      ...
      QryPartidas.Params.ParamByName('Contrato').Value := param_Global_Contrato;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            Connection.qryBuscaTrx.Params.ParamByName('Contrato').Value := param_global_contrato;
...
begin
  tiFase.Color := global_color_entradaERP
end;

procedure TfrmBitacoraxAlcance.tiFaseExit(Sender: TObject);
begin
  tiFase.Color := global_color_salidaERP;
...
begin
  ...
  begin
    ...
    qryResumendeAlcances.Params.ParamByName('contrato').Value := param_global_contrato;
    ...
    Alcances.Params.ParamByName('contrato').Value := param_global_contrato;
...
begin
  try
    ...
    if OrdenesdeTrabajo.RecordCount > 1 then
      cfnCalculaAvances(param_global_contrato, tsNumeroOrden.Text, convenio_reporte, 'XXX', True, tdIdFecha.Date, 'Avanzada', frmBitacoraxAlcance)
    else
      cfnCalculaAvances(param_global_contrato, tsNumeroOrden.Text, convenio_reporte, 'XXX', False, tdIdFecha.Date, 'Avanzada', frmBitacoraxAlcance);
...
begin
  ...
  if frmBarra1.btnCancel.Enabled = True then
    ...
    begin
      ...
      Connection.qryROProrrateos.ParamByName('contrato').AsString := param_global_contrato;
...
begin
  try
    ...
    if tsNumeroActividad.Text <> '' then
      procFichaTecnica(param_global_contrato, global_convenio, tsNumeroActividad.Text, frmBitacoraxAlcance);
  except
    on e: exception do
    begin
...
begin
  try
    //Traemos las descripciones y descripciones de contrato de acuerdo al parametro..
    ActualizaConfiguraciones(param_global_contrato);
    procAnalisisFinanciero(param_global_contrato, tsNumeroOrden.Text, convenio_reporte, tdIdFecha.Date, tdIdFecha.Date, frmBitacoraxAlcance, rDiario.OnGETValue, connection.configuracion.fieldbyname('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP))
  except
    on e: exception do
    begin
