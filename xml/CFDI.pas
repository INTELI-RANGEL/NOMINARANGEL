...
uses
  ...
  ClaseCertificadoSellos, SelloDigital;
...
begin
  ...
  fXMLComprobante.Attributes['xmlns:nomina'] := 'http://www.sat.gob.mx/nomina';  //*Nueva version XMl sat*/
  ...
  fXMLComprobante.Attributes['xsi:schemaLocation'] := 'http://www.sat.gob.mx/cfd/3 http://www.sat.gob.mx/sitio_internet/cfd/3/cfdv32.xsd http://www.sat.gob.mx/nomina http://www.sat.gob.mx/sitio_internet/cfd/nomina/nomina11.xsd';
...
begin
  ...
  Try
    ...
    fXMLComprobante.Attributes['certificado'] := SellosCSD.ComoBase64;
...
begin
  if fXMLGenerado then begin
    if fCadenaOriginalCalculada = '' then begin
      fFacturaXML.Active := True;
      fFacturaXML.Version := '1.0';           //*
      fFacturaXML.Encoding := 'utf-8';         //*
...
begin
  ...
  fFacturaXML.Version := '1.0';           //*
  fFacturaXML.Encoding := 'utf-8';
  if Folio.Serie <> '' then begin
    fXMLComprobante.Attributes['serie'] := Folio.Serie;
  end;
...
begin
  ...
  if cReceptor.ComplementoNomina.TipoDeContrato <> '' then begin
    fXMLComplemento.Attributes['TipoContrato'] := '02'; //cReceptor.ComplementoNomina.TipoDeContrato;
  end;
