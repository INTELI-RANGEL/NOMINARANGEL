...
uses
  ...
  cxDBFilterControl, FormAutoScaler, dxLayoutContainer, dxLayoutControl,
  dxLayoutcxEditAdapters, cxSpinEdit, cxStyles, cxCustomData, cxFilter, cxData,
  cxDataStorage, cxNavigator, cxDBData, cxGridLevel, cxClasses,
  cxGridCustomView, cxGridCustomTableView, cxGridTableView, cxGridDBTableView,
  cxGrid, cxGridCustomLayoutView, cxGridCardView, cxGridDBCardView, cxLabel;
...
type
  TfrmSetupAlm = class(TForm)
    ...
    Label7: TLabel;
    cxContacto: TcxComboBox;
    configuracionsContacto: TStringField;
    cxGroupBox5: TcxGroupBox;
    bImagenCalidad: TImage;
    Label8: TLabel;
    configuracionbImagenCalidad: TBlobField;
    cTsEntradas: TcxTabSheet;
    cxGrid1Level1: TcxGridLevel;
    cxGrid1: TcxGrid;
    cxGrid1DBCardView1: TcxGridDBCardView;
    QMascaras: TZQuery;
    dsMascaras: TDataSource;
    ...
    procedure bImagenCalidadClick(Sender: TObject);
    procedure LoadMascaras(Tipo:String);
    Procedure OpenMascaras(Tipo:String);


  private
    { Private declarations }
    FileQuality,FileCompany:TFileName;
...
{$R *.dfm}
procedure TfrmSetupAlm.OpenMascaras(Tipo:String);
begin
  QMascaras.Active:=False;
  if connection.contrato.FieldByName('sTipoObra').AsString='PRINCIPAL' then
    QMascaras.ParamByName('contrato').AsInteger:=-1
  else
    QMascaras.ParamByName('contrato').AsString:=global_contrato;
  QMascaras.ParamByName('tipo').AsString:=tipo;
  QMascaras.Open;
end;

procedure TfrmSetupAlm.LoadMascaras(Tipo:String);
var
  QIMascaras:TZQuery;
  QrContratos:TZReadOnlyQuery;
begin
  QIMascaras:=TZQuery.Create(nil);
  QrContratos:=TZReadOnlyQuery.Create(nil);
  try
    QIMascaras.Connection:=connection.zConnection;
    QrContratos.Connection:=connection.zConnection;

    QIMascaras.SQL.Text:='select * from mascaras where (:contrato=-1 or (:Contrato<>-1 and sContrato=:Contrato)) and eTipo=:Tipo';
    if connection.contrato.FieldByName('sTipoObra').AsString='PRINCIPAL' then
      QIMascaras.ParamByName('contrato').AsInteger:=-1
    else
      QIMascaras.ParamByName('contrato').AsString:=global_contrato;
    QIMascaras.ParamByName('Tipo').AsString:=Tipo;
    QIMascaras.Open;

    if connection.contrato.FieldByName('sTipoObra').AsString='PRINCIPAL' then
    begin
      QrContratos.SQL.Text:='select * from contratos where sTipoObra<>"PRINCIPAL"';
      QrContratos.Open;
      while not QrContratos.Eof do
      begin
        if not QIMascaras.Locate('sContrato;etipo',VarArrayOf([QrContratos.FieldByName('sContrato').AsString,tipo]),[]) then
        begin
          QIMascaras.Append;
          QIMascaras.FieldByName('sContrato').AsString:=QrContratos.FieldByName('sContrato').AsString;
          QIMascaras.FieldByName('etipo').AsString:=tipo;
          QIMascaras.FieldByName('sMascara').AsString:=QrContratos.FieldByName('sContrato').AsString + '-';
          QIMascaras.FieldByName('iConsecutivo').AsInteger:=1;
          QIMascaras.Post;
        end;
      ...
      if QIMascaras.RecordCount=0 then
      begin
        QIMascaras.Append;
        QIMascaras.FieldByName('sContrato').AsString:=global_contrato;
        QIMascaras.FieldByName('etipo').AsString:=tipo;
        QIMascaras.FieldByName('sMascara').AsString:=global_contrato + '-';
        QIMascaras.FieldByName('iConsecutivo').AsInteger:=1;
        QIMascaras.Post;
      end;
  finally
  ...
  begin
    ...
    begin
      try
        ...
        FileCompany:= OpenPicture.FileName;
      except
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        try
          BlobField := configuracion.FieldByName('bImagenCalidad');
          BS := configuracion.CreateBlobStream(BlobField, bmWrite);
          try
          Pic := TJpegImage.Create;
            try
              Pic.LoadFromFile(FileQuality);
              Pic.SaveToStream(Bs);
            finally
              Pic.Free;
            end;
      ...
      Configuracion.FieldByName('sContacto').AsString         := cxContacto.Text ;
      configuracion.Post;
...
begin
  ...
  begin
    ...
    cxContacto.Text := configuracion.FieldByName('sContacto').AsString ;
    ...
    begin
      ...
      begin
          ...
          try
            Pic.LoadFromStream(bS);
            bImagenCalidad.Picture.Graphic := Pic;
          finally
            Pic.Free;
          end;
        finally
        ...
        bImagenCalidad.Picture.LoadFromFile('');
...
procedure TfrmSetupAlm.bImagenCalidadClick(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPicture.Title := 'Inserta Imagen';
    ...
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPicture.FileName);
        FileQuality:= OpenPicture.FileName;
      except
        TImage(Sender).Picture.LoadFromFile('');
        FileQuality:='';
      end
