...
type
  TfrmMovimientosInsumos = class(TForm)
    ...
    procedure btnAfectadasClick(Sender: TObject);
  private
    sMenuP: string;
...
begin
  try
    ...
    qryInsumos.SQL.Add('select i.sIdInsumo,substr(i.mDescripcion,1,200) as mDescripcion,i.dExistencia, f.sDescripcion, i.sIdGrupo from insumos i ' +
                       'Inner Join familias f On (f.sIdFamilia=i.sIdGrupo)  ' +
                       'INNER JOIN almacenes AS b ON (i.sIdAlmacen = b.sIdAlmacen) ' +
                       'WHERE (:contrato=-1 or (:contrato<>-1 and i.sContrato = :contrato)) ' +
                       'and (:Principal=-1 or (:Principal<>-1 and b.lPrincipal=:Principal))' +
                       'Order by i.sIdInsumo,i.mDescripcion') ;

    If Connection.configuracion.fieldValues['sAlmcon'] = 'CONTRATOS' Then
    begin
      qryInsumos.ParamByName('Principal').AsInteger:=-1 ;
      qryInsumos.ParamByName('Contrato').AsString := global_contrato;
    end
    else
    begin
      qryInsumos.ParamByName('Principal').AsString:='Si';
       qryInsumos.ParamByName('Contrato').AsInteger:=-1;
    end;
    qryInsumos.Open;

    if qryInsumos.RecordCount = 0 then
...
procedure TfrmMovimientosInsumos.btnAfectadasClick(Sender: TObject);
var
...
begin
    ...
    begin
        ...
        qryReporteInsumos.SQL.Text:='select i.*,reco.iFolioRequisicion,orco.iFolioPedido from insumos i ' +
                                    'left join anexo_prequisicion reco '+
                                    'on(reco.sContrato=:ContratoActual and reco.sIdInsumo=i.sidinsumo) ' +
                                    'left join anexo_ppedido orco '+
                                    'on(orco.sContrato=:ContratoActual and orco.sIdInsumo=i.sidinsumo) '+
                                    'where (:contrato=-1 or (:contrato<>-1 and i.sContrato=:contrato)) '+
                                    'and sIdGrupo like :familia and ( '+
                                   // '(((:Reco=-1) or (:Reco<>-1 and not reco.iFolioRequisicion  is null ))  and '+
                                  //  '((:=-1) or (:orco<>-1 and not orco.iFoliopedido  is null ))) ' +
                                    '(((:Reco=-1) or (:Reco<>-1 and :Orco=-1 and not reco.iFolioRequisicion  is null )) '+
                                    'and ((:orco=-1) or (:orco<>-1 and :Reco=-1 and not orco.iFoliopedido  is null ))) '+
                                    'or ((:reco<>-1 and :orco<>-1) and (not orco.iFoliopedido  is null or not reco.iFolioRequisicion  is null )) '+
        ...
        qryReporteInsumos.ParamByName('ContratoActual').AsString  := global_contrato;

        if chkRequisicion.Checked then
           qryReporteInsumos.ParamByName('reco').AsInteger :=1
        else
           qryReporteInsumos.ParamByName('reco').AsInteger :=-1;

        if chkOrdenCompra.Checked then
           qryReporteInsumos.ParamByName('orco').AsInteger :=1
        else
           qryReporteInsumos.ParamByName('orco').AsInteger :=-1;
...
begin
  ...
  begin
      ...
      qryReporteInsumos.Active := false;
      qryReporteInsumos.SQL.Text:='select *  from insumos ' +
                                  'where (:contrato=-1 or (:contrato<>-1 and sContrato=:contrato)) '+
                                  'and sIdGrupo like :familia and sIdInsumo like :insumo ';
