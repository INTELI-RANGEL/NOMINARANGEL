...
uses
  ...
  IdComponent, IdTCPConnection, IdTCPClient, Sockets, DB, strUtils, ADOdb,
  GradienteM, Newpanel, ExtCtrls, jpeg, ComCtrls, ZDataset, frxpngimage,
  AdvGlassButton, IniFiles, ZSqlProcessor, UnitTIniTracer, Menus, OleCtrls,
  ShockwaveFlashObjects_TLB, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters,
  dxSkinsCore, dxSkinOffice2007Black, dxSkinOffice2007Blue,
  dxSkinOffice2007Green, dxSkinOffice2007Pink, dxSkinOffice2007Silver,
  dxSkinOffice2010Black, dxSkinOffice2010Blue, dxSkinOffice2010Silver,
  dxSkinOffice2013White, cxButtons, dxSkinsdxBarPainter, cxClasses, dxBar,
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if OldDB = zQuery.FieldByName( 'database' ).AsString then
...
begin
  try
    ...
    try
      ...
      try
        OldtsPuerto := StrToInt(VarRegistry('\Ventanas', '\' + Self.Name, 'tsPuerto'));
      except
        OldtsPuerto := StrToInt(tsPuerto.Text);
      end;

      tsPuerto.Text := IntToStr(OldtsPuerto);
        ...
        try
          ...
          if (MiArchivo = '') or (not fileExists(MiArchivo)) then begin
            ...
            if IniTracer.CambiarIni = '' then
              raise Exception.Create('La aplicación no puede funcionar sin archivo de configuración, por lo tanto se cerrará')
            else
              raise Exception.Create('Es necesario volover a iniciar la aplicación para que el cambio de archivo de configuracion tenga efecto');
          end;
        finally
          IniTracer.Free;
        end;
...
begin
  SetRegistry('\Ventanas', '\' + Self.Name, 'tsPuerto', tsPuerto.Text);
