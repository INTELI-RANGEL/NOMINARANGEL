...
uses
  ...
  cxGroupBox, dxdbtrel, SysUtils, cxDBEdit;
type
...
type
  ...
  dsProfesiones: TDataSource;
    ...
    Label10: TLabel;
    cxContratos: TcxComboBox;
    zqRptEntregaEquipoCategoria: TStringField;
    UdSqlEquiposporempleado: TZUpdateSQL;
  ...
  procedure Button7Click(Sender: TObject);
    ...
    procedure BView_EmpleadosKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure BView_EmpleadosKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure BView_EmpleadosCellClick(Sender: TcxCustomGridTableView;
      ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
      AShift: TShiftState; var AHandled: Boolean);
    procedure cxContratosExit(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      ShellExecute(Handle,'open',pWidechar(dlgSaveWord.FileName+zWord.fieldbyname('sext').asstring), nil, nil,  SW_SHOWNORMAL);
    end
    else
    begin
...
procedure Tfrm_Empleados.BView_EmpleadosCellClick(
...
procedure Tfrm_Empleados.BView_EmpleadosKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  llenarCampos() ;
end;

procedure Tfrm_Empleados.BView_EmpleadosKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  llenarCampos() ;
end;

procedure Tfrm_Empleados.cxContratosExit(Sender: TObject);
begin
// tsContrato.Text := cxContratos.Text ;
end;
...
begin
  ...
  try
    ...
    zQEmpleados.Params.ParamByName('Contrato').DataType := ftString ;
    zQEmpleados.Params.ParamByName('Contrato').Value    := Global_Contrato ;
    zQEmpleados.Open;
  finally
  ...
  ActualizaLibretas;
    ...
    while not zqContratos.Eof do
     begin
       cxContratos.Properties.items.Add(zqContratos.FieldByName('sContrato').AsString) ;
       zqContratos.Next ;
     end;
  cxContratos.Text := zQEmpleados.fieldByName('sContrato').AsString ;
end;
                  ...
                  //zQEmpleados.Delete;
                   With connection.zCommand do
                      begin
                        Active := False;
                        Sql.Clear ;
                        Sql.Add('DELETE FROM empleados Where sIdEmpleado =:id') ;
                        ParamByName('id').AsString       := tsIdEmpleado.Text ;
                        ExecSQL ;
                      end ;
                   zQEmpleados.Refresh ;
...
begin
 // try
    ...
    zQEmpleados.FieldByName('sContrato').AsString     := cxContratos.Text ;
    ...
    ActualizaCuenta;
     ...
     With connection.zCommand do
       begin
         Active := False;
         Sql.Clear ;
         Sql.Add('update empleados set sContrato =:Contrato Where sIdEmpleado =:id') ;
           ParamByName('Contrato').AsString := cxContratos.Text ;
           ParamByName('id').AsString       := tsIdEmpleado.Text ;
         ExecSQL ;
        end ;
  zQEmpleados.Refresh;
...
begin
    ...
    connection.QryBusca.SQL.Add('insert into nuc_personal '+
                                ...
                                'fechanacimiento,sexo,IdPlazaDetalle) '+
                                'values '+
                                ...
                                ':municipio, :numero, :pais, :idcargo, :idplaza, :estadociv, :cuidadnac,"No", :numinfonavit,0.00, '+
                                ':fechanac, :sexo, :idplaza)');
...
begin
 ...
 procedure Tfrm_Empleados.llenarCampos() ;
 Var
  QryBusca: TZQuery;
  BlobField: TField;
  Bs: TStream;
