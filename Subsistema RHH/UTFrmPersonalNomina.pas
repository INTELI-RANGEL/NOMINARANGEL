...
type
  TFrmPersonalNomina = class(TForm)
    ...
    cdSeleccionadosTituloDepartamento: TStringField;
    cdDisponiblesTituloDepartamento: TStringField;
    DetalleTituloDepartamento: TcxGridDBColumn;
...
//        raise InteligentException.CreateByCode(24, ['Debe seleccionar por lo menos un registro de la tabla para poder agregar.']);
      ...
      begin
        ...
        begin
          cdDisponibles.Locate('IdPersonal',cxGridDbDetalle.viewData.Records[i].Values[cxGridDbDetalleIdPersonal1.Index] ,[]);
          ...
          cdSeleccionados.FieldByName('IdDepartamento').AsInteger := cdDisponibles.FieldByName('IdDepartamento').AsInteger;
          cdSeleccionados.FieldByName('TituloDepartamento').AsString := cdDisponibles.FieldByName('TituloDepartamento').AsString;
  ...
  try
    ...
    Try
      ...
      begin
        ...
        cdDisponibles.FieldByName('IdDepartamento').AsInteger := cdSeleccionados.FieldByName('IdDepartamento').AsInteger;
        cdDisponibles.FieldByName('TituloDepartamento').AsString := cdSeleccionados.FieldByName('TituloDepartamento').AsString;
...
procedure TFrmPersonalNomina.cxGridDbDetalleStylesGetContentStyle(
  ...
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
  if VarToSTR(ARecord.Values[cxGridDbDetalleEstado1.Index]) = '-1' then
  ...
  if (ARecord.values[CxColumnEspecial.Index] = 'Si') and ((VarToSTR(ARecord.Values[cxGridDbDetalleEstado1.Index]) = '-1') or (VarToStr(ARecord.Values[cxGridDbDetalleEstado1.Index]) = '1')) then
...
begin
  try
  if VarToStr(ARecord.Values[cxGridDbSeleccionadosEstado1.Index]) = '0' then
  //if VarToStr(ARecord.Values[7]) = '0' then
    AStyle := cxstylBoldBlack;
  except
    on e:Exception do
      ShowMessage('Err1: ' + e.Message);
  end;
  ...
  try
  if VarToStr(ARecord.Values[CxColumnPagado.Index]) = '1' then
    AStyle := cxstylPagado;
  except
    on e:Exception do
      ShowMessage('Err3: ' + e.Message);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cdDisponibles.FieldByName('TituloDepartamento').AsString := dsPersonalIncluido.DataSet.FieldByName('TituloDepartamento').AsString;
    ...
    begin
      ...
      cdSeleccionados.FieldByName('TituloDepartamento').AsString := dsPersonalIncluido.DataSet.FieldByName('TituloDepartamento').AsString;
