...
uses
  ...
  dxSkinscxPCPainter, cxGridLevel, cxCurrencyEdit, RxToolEdit, RxCurrEdit,
  RxDBCurrEdit;
...
type
  ...
  TFrmContratarWizard = class(TForm)
    ...
    procedure LCbbCargoPropertiesChange(Sender: TObject);
  private
...
begin
  ...
  try
    ...
    begin
      if cmbOrganizacion.ItemIndex >= 0 then
  ...
  if cmbOrganizacion.ItemIndex >= 0 then
  begin
    if (Not dsCargo.DataSet.Filtered) or
       (dsCargo.DataSet.FieldByName('IdOrganizacion').AsInteger <> TOrganizacion(cmbOrganizacion.Properties.Items.Objects[cmbOrganizacion.ItemIndex]).Identificador) or
       (dsCargo.DataSet.FieldByName('IdTipoNomina').AsInteger <> dsTipoNomina.DataSet.FieldByName('IdTipoNomina').AsInteger) then
    begin
  ...
  try
    ...
    try
      GenerarCalculoVirtual(Self, StrToFloat(edtNeto.Text), cdTmpOrganizacion.FieldByName('IdOrganizacion').AsInteger,
                            ...
                            dsTipoNomina.DataSet.FieldByName('IdTipoNomina').AsInteger, dsDepartamento.DataSet.FieldByName('IdDepartamento').AsInteger, dsTipoNomina.DataSet.FieldByName('DiasTrabajados').AsInteger,
                            dsTipoNomina.DataSet.FieldByName('DiasDescanso').AsInteger, StrToInt(advLblNoFestivos.Caption.Text), cdGrupoSalario.FieldByName('Domingos').AsInteger,
                            dsImss.DataSet.FieldByName('Jornada').AsInteger,dsTipoNomina.DataSet.FieldByName('InicioSemana').AsString, dsTipoNomina.DataSet.FieldByName('IdTipoTablaISPT').AsInteger,
  ...
  try
    try
      ...
      GenerarCalculoVirtual(Self, -1,
                            ...
                            dsDepartamento.DataSet.FieldByName('IdDepartamento').AsInteger,
...
{      end
      ...
      cdBajaImss.FieldByName('idcargo').AsInteger := dsImss.DataSet.FieldByName('idcargo').AsInteger;
      cdBajaImss.FieldByName('idDepartamento').AsInteger := dsImss.DataSet.FieldByName('idDepartamento').AsInteger;
      ...
      begin
        ...
        try
          try
            ...
            dsImss.DataSet.FieldByName('IdOrganizacion').AsInteger := Globales.Elemento('IdOrganizacion').AsInteger;
...
procedure TFrmContratarWizard.LCbbCargoPropertiesChange(Sender: TObject);
begin
  ceSalario.Visible := True;
end;
