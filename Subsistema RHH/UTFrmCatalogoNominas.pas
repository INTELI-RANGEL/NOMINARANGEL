...
uses
  ...
  AdvSmoothPopup, AdvOfficeHint, cxDBExtLookupComboBox, cxSpinEdit, DBGrids;
...
type
  ...
  TFrmCatalogoNominas = class(TDxRibbonForm)
    ...
    CambioDepartamento1: TMenuItem;
    pnlCambioDepto: TPanel;
    cxGroupBox1: TcxGroupBox;
    cbbNuevoDepto: TcxComboBox;
    cxLabel4: TcxLabel;
    lblCabecera: TcxLabel;
    btnCambioDeptoOk: TcxButton;
    btnCambioDeptoCancel: TcxButton;
    dsPersonalDetalleNomina: TDataSource;
    btnListaRaya: TdxBarLargeButton;
    ListaRaya: TfrxReport;
    FDSListaRaya: TfrxDBDataset;
    cdListaRaya: TClientDataSet;
    FDSNomina: TfrxDBDataset;
    ...
    procedure btnOkAlternoClick(Sender: TObject);
    ...
    procedure CambioDepartamento1Click(Sender: TObject);
    procedure btnCambioDeptoOkClick(Sender: TObject);
    procedure btnCambioDeptoCancelClick(Sender: TObject);
    ...
    Try
      ...
      begin
        if Not CrearConjunto(cdPersonalDetalleNomina, 'nuc_personal_detallenomina', ccCatalog) then
          raise InteligentException.CreateByCode(5, ['Detalle de Nómina por Personal']);
      end
      else
...
{          //por cuestiones del destino en mi pc el cdconcepto aparece como inactivo, es por eso q se abre aqui
          ...
          *)
          ...
          begin
            ...
            begin
              ...
              Try
                ...
                memDeptoPercepciones.Filter := 'idagrupador = ' + QuotedStr(cdDetalleLoad.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdDetalleSave.FieldByName('IdOrganizacion').AsString*) + ' and CodigoConcepto = ' + QuotedStr(Codigo);
                ...
                begin
                  ...
                  memDeptoPercepciones.FieldByName('idAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').AsString;
                ...
                begin
                  ...
                  memDeptoPercepciones2.FieldByName('idAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').AsString;
          ...
          begin
            ...
            Try
              ...
              memDeptoDeducciones.Filter := 'idagrupador = ' + QuotedStr(cdDetalleLoad.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdDetalleSave.FieldByName('IdOrganizacion').AsString *) + ' and CodigoConcepto = ' + QuotedStr(Codigo);
              ...
              begin
                ...
                memDeptoDeducciones.FieldByName('idAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').AsString;
              ...
              begin
                ...
                memDeptoDeducciones2.FieldByName('idAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').AsString;
          ...
          begin
            ...
            Try
              ...
              memObligacion.Filter := 'IdAgrupador = ' + QuotedStr(cdDetalleLoad.FieldByName('IdDepartamento').AsString) (* + ' and IdOrganizacion = ' + cdDetalleSave.FieldByName('IdOrganizacion').AsString*) +' and CodigoConcepto = ' + QuotedStr(Codigo);
              memObligacion.Filtered := True;

              if memObligacion.RecordCount = 0 then
              begin
                memObligacion.Append;
                memObligacion.FieldByName('IdAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').asString;
          ...
          begin
            Try
              ...
              memRubrosIMSS.Filter := 'IdAgrupador = ' + QuotedStr(cdDetalleLoad.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdDetalleSave.FieldByName('IdOrganizacion').AsString *)+ ' and IdGrupoConcepto = ' + Grupo;
              memRubrosIMSS.Filtered := True;

              if memRubrosIMSS.RecordCount = 0 then
              begin
                memRubrosIMSS.Append;
                memRubrosIMSS.FieldByName('IdAgrupador').AsString := cdDetalleLoad.FieldByName('IdDepartamento').AsString;
    ...
    Finally
      ...
      EliminarConjunto(cdDetalleLoad);
...
begin
  ...
  try
    ...
    try
      ...
      if Assigned(NominaPersonal) then
      begin
        EliminarConjunto(NominaPersonal);
...
begin
  try
    // Verificar que exista una nómina abierta
    if Not cdNomina.Active or (cdNomina.RecordCount = 0) then
      raise InteligentException.CreateByCode(24, ['Debe abrir primero una nómina']);
...
begin
  ...
  begin
    ...
    cdTimbrado.Filtered := True;
      ...
      memDeptoPercepciones.Filtered := False;
      memDeptoPercepciones.Filter := 'IdAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+  ' and IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
      memDeptoPercepciones.Filtered := True;

      memDeptoDeducciones.Filtered := False;
      memDeptoDeducciones.Filter := 'IdAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
      memDeptoDeducciones.Filtered := True;

      memObligacion.Filtered := False;
      memObligacion.Filter := 'IdAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
      memObligacion.Filtered := True;

      memRubrosIMSS.Filtered := False;
      memRubrosIMSS.Filter := 'IdAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+ ' and 'IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
      memRubrosIMSS.Filtered := True;

    {$REGION 'Recibo'}
    memDeptoPercepciones2.Filtered := False;
    memDeptoPercepciones2.Filter := 'IdAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+  ' and IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
    memDeptoPercepciones2.Filtered := True;

    memDeptoDeducciones2.Filtered := False;
    memDeptoDeducciones2.Filter := 'idAgrupador = ' + QuotedStr(cdPersonalDetalleNomina.FieldByName('IdDepartamento').AsString) (*+ ' and IdOrganizacion = ' + cdPersonalDetalleNomina.FieldByName('IdOrganizacion').AsString*);
    memDeptoDeducciones2.Filtered := True;
...
begin
  ...
  EliminarConjunto([cdConcepto, cdConceptoExtra, cdDesactivarTimbre, cdDetalleGrafica, cdEncabezadoXLS, cdExcepcionesCat, cdExcepcionesUpt, cdFestivos, cdFiniquitoDed, cdFiniquitoPer, cdGravados, cdGruposConcepto, CdHorasExtras, cdInasistenciaUpt, cdMachotes, cdModuleOrganizacion, cdPersonal, cdSumaConcepto, cdTimbrado, cdTipoInasistencia, Cortes]);
...
begin
  ...
  begin
    ...
    ConsultarVacaciones1.Visible := (AnsiCompareText(cdTipoNomina.FieldByName('TipoCalculo').AsString, 'FINIQUITOS') = 0) OR (AnsiCompareText(cdTipoNomina.FieldByName('TipoCalculo').AsString, 'ESPECIAL') = 0);
  end;
end;
...
begin
  ...
  if CompareText(VarName, 'CANTIDADLETRA2') = 0 then
    Value := UpperCase(LetrasNumeros((TotalPercepciones - TotalDeducciones), 'PESOS', '/100 M.N.', 2)) ;
  ...
  try
    ...
    begin
      ...
      try
        ...
        if Assigned(cdPagosNominas) then
          EliminarConjunto(cdPagosNominas);
...
procedure TFrmCatalogoNominas.CambioDepartamento1Click(Sender: TObject);
var
  CdPlazaDetalle, CdAltaPerImms : TclientDataset;
  OldCursor: TCursor;
  IdPersonal : integer;
  FormaCambio: TForm;
  cdDep: TClientDataSet;
begin
  try
    try
      cdDep := TClientDataSet.Create(Self);
      if Not CrearConjunto(cdDep, 'nuc_departamento', ccCatalog) then
        raise InteligentException.CreateByCode(5, ['Departamentos']);

      cdDep.Open;

      cbbNuevoDepto.Properties.Items.Clear;
      while not cdDep.Eof do
      begin
        cbbNuevoDepto.Properties.Items.AddObject(cdDep.FieldByName('TituloDepartamento').AsString, TObject(cdDep.FieldByName('IdDepartamento').AsInteger));
        cdDep.Next;
      end;

      cbbNuevoDepto.ItemIndex := 0;
    finally
      EliminarConjunto(cdDep);
    end;

    try
      // Pedir el nuevo departamento al usuario
      FormaCambio := TForm.Create(Self);
      FormaCambio.ModalResult := mrOk;
      FormaCambio.Width := pnlCambioDepto.Width + 12;
      FormaCambio.Height := pnlCambioDepto.Height + 45;
      FormaCambio.BorderStyle := bsDialog;
      FormaCambio.Position := poOwnerFormCenter;
      lblCabecera.Caption := 'Actual: ' + GridPersonal.Cell[3, GridPersonal.SelectedRow].AsString;
      pnlCambioDepto.Parent := FormaCambio;
      pnlCambioDepto.Align := alClient;
      pnlCambioDepto.Visible := True;
      if FormaCambio.ShowModal = mrOk then
        AbrirPersonalIncluido(cdPersonalIncluido, cdNomina.FieldByName('IdNomina').asInteger, False);
    finally
      pnlCambioDepto.Visible := False;
      pnlCambioDepto.Align := alNone;
      pnlCambioDepto.Parent := Self;
      FormaCambio.Free;
    end;
  except
    on e:inteligentexception do
    ...
    try
      ...
      begin
        ...
        begin
          ...
          sCodigo.Valor := GridPersonal.Cell[9, i].AsString;
          pLista := pLista + GridPersonal.Cell[9, i].AsString + ',';
          nNombre := StringReplace(GridPersonal.Cell[1, i].AsString, ',', '', [rfReplaceAll]);
          FrmPagoNomina.lbPersonal.AddItem(GridPersonal.Cell[0, i].AsString + ' | ' + nNombre + ' | ' + GridPersonal.Cell[8, i].AsString, sCodigo);
...
procedure TFrmCatalogoNominas.btnCambioDeptoCancelClick(Sender: TObject);
begin
  TForm(btnCambioDeptoCancel.Parent.Parent).ModalResult := mrCancel;
  TForm(btnCambioDeptoCancel.Parent.Parent).Close;
end;

procedure TFrmCatalogoNominas.btnCambioDeptoOkClick(Sender: TObject);
var
  cdPN, cdResultado: TClientDataSet;
begin
  cdPN := TClientDataSet.Create(Self);
  cdResultado := TClientDataSet.Create(Self);
  try
    if Not CrearConjunto(cdPN, 'nuc_personal_nomina', ccCatalog) then
      raise InteligentException.CreateByCode(16, ['Personal para Nómina']);


    CargarDatosFiltrados(cdPN, 'IdNomina,IdPersonal', [cdNomina.FieldByName('IdNomina').AsInteger, GridPersonal.Cell[9, GridPersonal.SelectedRow].AsInteger]);
    cdPN.Open;

    if cdPN.RecordCount = 0 then
      raise InteligentException.CreateByCode(27, ['Personal en Periodo de Nómina', GridPersonal.Cell[9, GridPersonal.SelectedRow].AsInteger]);

    // Modificar el Departamento en el registro de nuc_personalimss, para ello se deberá verificar si existe un
    // registro con la fecha de inicio de la nómina y si no existe se deberá crear
    ProcedimientoAlmacenado(cdResultado, 'Res_CambioDepto',
                            'pIdPersonal,pFecha,pIdDepartamento,pJornada,pCobraFestivos,pRegistroPatronal,pSalarioDiario,pSalarioIntegrado,pIdCargo,pIdTipoNomina,pIdOrganizacion',
                            [GridPersonal.Cell[9, GridPersonal.SelectedRow].AsInteger,
                             ClientModule1.DatetoStrSql(cdNomina.FieldByName('FechaNomina').AsDateTime),
                             Integer(cbbNuevoDepto.Properties.Items.Objects[cbbNuevoDepto.ItemIndex]),
                             cdPN.FieldByName('Jornada').AsInteger,
                             cdPN.FieldByName('CobraFestivos').AsString,
                             //cdPN.FieldByName('IdPlazadetalle').AsVariant,
                             cdPN.FieldByName('IDPatronal').AsString,
                             cdPN.FieldByName('Salario').AsFloat,
                             cdPN.FieldByName('SalarioIntegrado').AsFloat,
                             //cdNomina.FieldByName('IdPeriodosGuardia').AsVariant,
                             cdPN.FieldByName('IdCargo').AsInteger,
                             cdNomina.FieldByName('IdTipoNomina').AsInteger,
                             cdPN.FieldByName('IdOrganizacion').AsInteger]);
    cdPN.Close;

    TForm(btnCambioDeptoCancel.Parent.Parent).ModalResult := mrOk;
    TForm(btnCambioDeptoCancel.Parent.Parent).Close;

    CalcularIndividual1Click(CalcularIndividual1);
  finally
    EliminarConjunto([cdPN, cdResultado]);
  end;
end;
...
procedure TFrmCatalogoNominas.btnListaRayaClick(Sender: TObject);
var
  FrmDeptos: TForm;
  Original: String;
begin
  ...
  Try
    ...
    Try
      ...
      tagbutton := 9; //TButton(sender).tag;
      ...
      Finally
        if (cdDeptos.ProviderName <> '') and cdDeptos.Active then
          cdDeptos.Close;
        EliminarConjunto(cdDeptos);
      End;

      btnOk.OnClick := btnOkAlternoClick;
      try
        frmDeptos.ShowModal;
      finally
...
procedure TFrmCatalogoNominas.btnOkAlternoClick(Sender: TObject);
var
  Objeto: TWinControl;
  locForma: TForm;
begin
  Try
    try
      if cdListaRaya.ProviderName = '' then
        if Not CrearConjunto(cdListaRaya, 'nom_listaraya', ccSelect) then
          raise InteligentException.CreateByCode(5, ['Lista de Raya']);

      if Not CargarDatosFiltrados(cdListaRaya, 'IdNomina,IdDepartamento', [cdNomina.FieldByName('IdNomina').AsInteger, Integer(cbbDeptos.Properties.Items.Objects[cbbDeptos.ItemIndex])]) then
        raise InteligentException.CreateByCode(6, ['Lista de Raya', Integer(cbbDeptos.Properties.Items.Objects[cbbDeptos.ItemIndex]), 'Id. Departamento']);

      if cdListaRaya.Active then
        cdListaRaya.Refresh
      else
        cdListaRaya.Open;

      ClientModule1.ImprimeReporte('ListaRaya.fr3', ListaRaya);
    finally
      if (cdListaRaya.ProviderName <> '') and cdListaRaya.Active then
      begin
        cdListaRaya.Close;
        EliminarConjunto(cdListaRaya);
      end;
...
{    if (tagButton <> 2) and (tagButton <> 9) then
    begin
      if cbbDeptos.ItemIndex = -1 then
      begin
        Objeto := cbbDeptos;
    ...
    begin
      IndiceDeptoOrg := Integer(cbbDeptos.Properties.Items.Objects[cbbDeptos.ItemIndex]);
      GenerarReporte('Recibo.fr3', ListaRaya);
    end;
...
var
  LocCursor: TCursor;
begin
  if ((AnsiCompareText(cdTipoNomina.FieldByName('TipoCalculo').AsString, 'FINIQUITOS') = 0) OR (AnsiCompareText(cdTipoNomina.FieldByName('TipoCalculo').AsString, 'ESPECIAL') = 0)) and (GridPersonal.SelectedRow >= 0) then
  begin
    Try
      ...
      Try
        ...
        Try
          ...
          *)
          ...
          //cdHistorial.Close;
          ProcedimientoAlmacenado(cdHistorial, 'Res_FechasVacaciones', 'pFecha,pIdPersonal,pAnio', [ClientModule1.DateToStrSQL(cdNomina.FieldByName('FechaTerminoNomina').AsDateTime), GridPersonal.Cell[9, GridPersonal.SelectedRow].AsInteger, -1]);
  ...
  try
    ...
    try
      ...
      if Assigned(Tipo) then
        EliminarConjunto(Tipo);

      if Assigned(mnom_total) then
        EliminarConjunto(mnom_total);
  ...
  try
    ...
    begin
      ...
      try
        ...
        if Assigned(cdtRegistros) then
          EliminarConjunto(cdTRegistros);
...
begin
  if Not cdNomina.Active or (cdNomina.RecordCount = 0) then
    raise InteligentException.CreateByCode(24, ['']);
...
begin
  try
    ...
    try
      ...
      if Not CargarDatosFiltrados(cdPersonalDetalleNomina, 'IdNomina', [cdNomina.FieldByName('IdNomina').AsInteger]) then
        raise InteligentException.CreateByCode(6, ['Personal por Detalle de Nómina', cdBuscar.FieldByName('IdNomina').AsInteger, 'IdNomina']);

      if cdPersonalDetalleNomina.Active then
        cdPersonalDetalleNomina.Refresh
      else
        cdPersonalDetalleNomina.Open;

      cdFiniquitoPer.MasterSource := dsPersonalDetalleNomina;
      cdFiniquitoPer.IndexFieldNames := 'IdPersonal;CveSat';
      cdFiniquitoPer.MasterFields := 'IdPersonal';

      cdFiniquitoDed.MasterSource := dsPersonalDetalleNomina;
      cdFiniquitoDed.IndexFieldNames := 'IdPersonal;CveSat';
      cdFiniquitoDed.MasterFields := 'IdPersonal';
      ProcederScroll := False;

      cdPersonalDetalleNomina.First;
      cdFiniquitoPer.First;
      cdFiniquitoDed.First;

      try
        ClientModule1.ImprimeReporte('reciboFiniquitos.fr3', FrxReportFiniquitos);
      finally
        cdFiniquitoPer.MasterFields := '';
        cdFiniquitoPer.IndexFieldNames := '';
        cdFiniquitoPer.MasterSource := Nil;

        cdFiniquitoDed.MasterFields := '';
        cdFiniquitoDed.IndexFieldNames := '';
        cdFiniquitoDed.MasterSource := Nil;
      end;
    finally
      Screen.Cursor := Cursor;
    end;
