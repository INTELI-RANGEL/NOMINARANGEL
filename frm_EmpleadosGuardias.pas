...
uses
  ...
  DateUtils, UDbGrid, dblookup, DBDateTimePicker, Newpanel, Buttons,
  frm_plataformas,frm_Pernoctan, RxMemDS, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, cxStyles, cxCustomData, cxFilter,
  cxData, cxDataStorage, cxEdit, cxNavigator, cxDBData, cxGridLevel,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxGridCustomView,
  cxGrid, cxClasses, cxContainer, cxTextEdit, dxBar, dxRibbonRadialMenu,
  cxCheckBox, cxDropDownEdit, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox,
  cxSpinEdit, cxTimeEdit, cxDBEdit, cxMaskEdit, cxCalendar, cxLabel, cxGroupBox,
  cxButtons, cxGridBandedTableView, cxGridDBBandedTableView, cxFilterControl,
  ...
  FormAutoScaler, DBClient, AdvSplitter, AdvDateTimePicker, dxSkinsCore,
  ...
  dxSkinscxPCPainter, dxSkinsdxBarPainter, ZSqlUpdate, JvExComCtrls,
  JvDateTimePicker, AdvCombo, Lucombo, dblucomb, AdvDBLookupComboBox,
  dxSkinMetropolis, dxSkinMetropolisDark, dxSkinOffice2013DarkGray,
  dxSkinOffice2013LightGray, cxSplitter, func_Genericas, FolderDialog,
  AdvSmoothSlider, UDosCapas;
...
type
  ...
  ConfirmarPersonal1: TMenuItem;
    ...
    ZQNomina_Guardia: TZQuery;Z: TcxTextEdit;
    cxButton3: TcxButton;
    ZQRGuardias: TZQuery;
    ...
    ContratoaBordoTodos1: TMenuItem;
    rutaContratos1: TFolderDialog;
  ...
  procedure ContratoaBordo1Click(Sender: TObject);
  ...
  procedure contratar;
    ...
    procedure cdNominaAfterScroll(DataSet: TDataSet);
...
private
  ...
  posicion, posicion_guardias, posicion_empleados : TBookmark;
...
procedure TfrmEmpleadosGuardias.odos1Click(Sender: TObject);
var
  i : Integer;
  QrDatos : TzReadOnlyQuery;
  filtro : string;
...
begin
  ...
  try
    ...
    QrDatos.SQL.Text:='select e.*,gm.*,nr.importe,rn.sDescripcion as nacionalidad' +
                      ...
                      'where gm.iIdGuardia=:Guardia';

    dtBaja:=False;
    QrDatos.ParamByName('Guardia').AsInteger:=zqryGuardiasMovtos.FieldByName('iIdguardia').asInteger;
    QrDatos.Open;

    Application.CreateForm(TfrmImprimeContratos, frmImprimeContratos);
    frmImprimeContratos.FormStyle := fsNormal;
    frmImprimeContratos.Visible := False;
    frmImprimeContratos.zQEmpleados.Open;

    frmImprimeContratos.FechaSubida := DateToStr(zqryEmpleados.fieldByName('dFechaSubida').AsDateTime);
...
begin
  ...
  begin
    ...
    begin
      ...
      arrayContratado[i] := BView_Empleados.Controller.SelectedRows[i].Values[tcxContratado.Index];//[BView_EmpleadossStatusPersonal.Index];
      arraysStatusTipoPersonal[i] := BView_Empleados.Controller.SelectedRows[i].Values[tcxsStatusTipoPersonal.Index];
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for i := 0 to BView_Empleados.Controller.SelectedRowCount - 1 do
      begin
        arraySStatusPersonal[i] := BView_Empleados.Controller.SelectedRows[i].Values[tcxContratado.Index];//[BView_EmpleadossStatusPersonal.Index];
        arraySIdEmpleados[i] := BView_Empleados.Controller.SelectedRows[i].Values[BView_EmpleadossIdEmpleado.Index];
...
procedure TfrmEmpleadosGuardias.cdNominaAfterScroll(DataSet: TDataSet);
begin
      ...
      begin
        ...
        begin
          ...
          try
            ...
            connection.QryBusca.ParamByName('NumeroOrden').AsString   := zqryOrdenes.FieldValues['sNumeroOrden'];
...
procedure TfrmEmpleadosGuardias.ContratoaBordo1Click(Sender: TObject);
var
  cursor : TCursor;
  ColumnaId : Integer;
  RegistroId : integer;
...
begin
  ...
  try
    ...
    QrDatos.SQL.Add('select' + #10 +
                    ...
                    'rn.sDescripcion as nacionalidad,' + #10 +
                    'nc.titulocargo,' + #10 +
                    'b.sDescripcion as sBanco' + #10 +
                    'from empleados e' + #10 +
                    'inner join guardiasmovtos gm' + #10 +
                    'on(gm.sIdEmpleado = e.sIdEmpleado)' + #10 +
                    'inner join nuc_personal np' + #10 +
                    'on(np.codigopersonal = e.sIdEmpleado)' + #10 +
                    'inner join nuc_cargo nc' + #10 +
                    'on(gm.iId_puesto=nc.idCargo)' + #10 +
                    'left join' + #10 +
                    'nom_nomina nom' + #10 +
                    'on (nom.IdPeriodosGuardia = gm.ID_GuardiaPeriodo)' + #10 +
                    'left join' + #10 +
                    'nom_nominapersonal as nop' + #10 +
                    'on (nop.IdNomina = nom.IdNomina and nop.IdPersonal = np.IdPersonal)' + #10 +
                    'left join nom_rangosalarios nr' + #10 +
                    'on(nr.IdRangoSalarios=nc.IdRangoSalarios)' + #10 +
                    'left join rh_nacionalidades rn ' + #10 +
                    'on(rn.iIdNacionalidad=e.iIdNacionalidad)' + #10 +
                    'left join rh_bancos b' + #10 +
                    'on (e.sIdBanco = b.sIdBanco)' + #10 +
                    'where' + #10 +
                    '(:iIdGuardiaMovtos = -1 or (:iIdGuardiaMovtos <> -1 and FIND_IN_SET(gm.iIdGuardiaMovtos, :iIdGuardiaMovtos))) and ' + #10 +
                    'nop.IdFormaPago is null and gm.Reprogramacion = "0000-00-00"');

    //obtener  la cadena  de Id de  guardias movimientos para imprimir reporte

    dtBaja:=False;
      if zqryEmpleados.Active then
        zqryEmpleados.Refresh;

      QrDatos.ParamByName('iidguardiamovtos').AsString := ListIdGuardiaMovtos;

    if QrDatos.Active then
      QrDatos.Refresh
    else
      QrDatos.Open;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          frmImprimeContratos.frx_ReporteEmpleados.DataSet := QrDatos;
          frmImprimeContratos.zQEmpleados.Open;
          frmImprimeContratos.zQEmpleados.Locate('sIdEmpleado', QrDatos.FieldByName('sIdEmpleado').AsString, []);
          //frmImprimeContratos.zQEmpleados.FieldByName('dSueldo').AsFloat := QrDatos.FieldByName('dSueldo').AsFloat;
        ...
        begin
          //Aqui  es  donde se imprime  el reporte
          ...
          frxContratos.LoadFromFile(Global_Files + global_miReporte +'_Contratos_ABordoF.fr3') ;
    {
...
begin
  ...
  begin
    rutaContratos := rutaContratos1.Directory;
...
begin
  ...
  begin
    ...
    {
    frxContratos.Variables.variables['SueldoMensualC']    :=  quotedstr(xNumerosToLletres(DataSet.FieldByName('importe').AsCurrency));
    frxContratos.Variables.variables['Contratacion_Dia']  :=   quotedstr(FormatDateTime('dd',DataSet.FieldByName('dFechaSubida').AsDateTime));
    frxContratos.Variables.variables['Contratacion_Mes']  := quotedstr(Uppercase(FormatDateTime('mmmm', DataSet.FieldByName('dFechaSubida').AsDateTime)));
    frxContratos.Variables.variables['Contratacion_Anyo'] := quotedstr(FormatDateTime('yy', DataSet.FieldByName('dFechaSubida').AsDateTime));
...
begin
  if (date < ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime) then
    ...
    ZQEmpleadosH.FieldByName('dias_trabajados').asInteger := (DaysBetween(ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime, ZQEmpleadosH.FieldByName('dFechaBajada').AsDateTime)+1)
    ...
    ZQEmpleadosH.FieldByName('dias_trabajados').asInteger := (DaysBetween(ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime, Date)+1)
  else if (date = ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime) and (date < ZQEmpleadosH.FieldByName('dFechaBajada').AsDateTime) then
    ZQEmpleadosH.FieldByName('dias_trabajados').asInteger := (DaysBetween(ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime, Date)+1)
  else if (date > ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime) and (date = ZQEmpleadosH.FieldByName('dFechaBajada').AsDateTime) then
    ZQEmpleadosH.FieldByName('dias_trabajados').asInteger := (DaysBetween(ZQEmpleadosH.FieldByName('dFechaSubida').AsDateTime, ZQEmpleadosH.FieldByName('dFechaBajada').AsDateTime)+1);
...
begin
  ...
  if (date < zqryEmpleados.FieldByName('dFechaSubida').AsDateTime) then
    ...
    zqryEmpleados.FieldByName('dias_trabajados').asInteger := (DaysBetween(zqryEmpleados.FieldByName('dFechaSubida').AsDateTime, zqryEmpleados.FieldByName('dFechaBajada').AsDateTime)+1)
    ...
    zqryEmpleados.FieldByName('dias_trabajados').asInteger := (DaysBetween(zqryEmpleados.FieldByName('dFechaSubida').AsDateTime, Date)+1)
  else if (date = zqryEmpleados.FieldByName('dFechaSubida').AsDateTime) and (date < zqryEmpleados.FieldByName('dFechaBajada').AsDateTime) then
    zqryEmpleados.FieldByName('dias_trabajados').asInteger := (DaysBetween(zqryEmpleados.FieldByName('dFechaSubida').AsDateTime, Date)+1)
  else if (date > zqryEmpleados.FieldByName('dFechaSubida').AsDateTime) and (date = zqryEmpleados.FieldByName('dFechaBajada').AsDateTime) then
    zqryEmpleados.FieldByName('dias_trabajados').asInteger := (DaysBetween(zqryEmpleados.FieldByName('dFechaSubida').AsDateTime, zqryEmpleados.FieldByName('dFechaBajada').AsDateTime)+1);
      ...
      try
        ...
        begin
          ...
          zqryEmpleados.SQL.add('select '+
                                ...
                                ' if(pg.FechaInicio >=(Select Max(IFNULL(FechaReal,FechaMovimiento)) From nuc_personalimss where idPersonal = np.IdPersonal and tipoMovimiento = "Baja"), "NoContratado","Contratado") as sStatusPersonal, '+
                                ...
                                'if (( ' +   //Agregado el 2015/03/17
                                'select ' +  //Agregado el 2015/03/17
                                '   npi.IdPeriodosGuardia ' + //Agregado el 2015/03/17
                                'from nuc_personalimss npi ' + //Agregado el 2015/03/17
                                'inner join nom_periodosguardia npg ' + //Agregado el 2015/03/17
                                'on (npg.IdPeriodosGuardia = npi.IdPeriodosGuardia) ' + //Agregado el 2015/03/17
                                'where ' + //Agregado el 2015/03/17
                                '   npg.IdPeriodosGuardia = pg.IdPeriodosGuardia and ' + //Agregado el 2015/03/17
                                '   npi.IdPersonal = np.IdPersonal and ' + //Agregado el 2015/03/17
                                '   npi.TipoMovimiento = "Baja" limit 1) = pg.IdPeriodosGuardia, "Contratado", "NoContratado") ' + //Agregado el 2015/03/17
    ...
    begin
      ...
      begin
        ...
        zqryGuardiasMovtos.SQL.add('select o.sNumeroOrden, '+
                                   ...
                                   'where (o.sContrato = :contrato and  o.sidplataforma = :sidplataforma and o.sNumeroOrden = :NumeroOrden) group by gd.iIdGuardia ORDER BY gd.ID_PeriodoGuardia ASC');
