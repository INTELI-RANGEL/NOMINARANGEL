...
uses  frm_isometricos, frm_trinomios, frm_connection, frm_comentariosxanexo, frm_EstimacionAlbum,
  ...
  frm_DespieceImagen, frm_ordenesctrlobra, frm_estimaciones;
...
begin
  ...
  begin
    ...
    if OrdenesdeTrabajo.Active then
      OrdenesdeTrabajo.Refresh;

    if connection.ds_estimacionperiodo.DataSet.Active then
      connection.ds_estimacionperiodo.DataSet.Refresh;
  end;
    ...
    begin
      ...
      begin //********************************************por esto no habre el reporte
        ...
        if QryConfiguracion.FieldValues['iFirmas'] = '2' then
          begin
          if not FileExists(global_files + global_miReporte + '_ResumenGenerador2.fr3') then
            showmessage('El archivo de reporte '+global_Mireporte+'_ResumenGenerador2.fr3 no existe, notifique al administrador del sistema')
          else
        ...
        if QryConfiguracion.FieldValues['iFirmas'] = '3' then
           if not FileExists(global_files + global_miReporte + '_ResumenGenerador.fr3') then
             showmessage('El archivo de reporte '+global_Mireporte+'_ResumenGenerador.fr3 no existe, notifique al administrador del sistema')
           else
...
begin
  try
    ...
    begin
      ...
      if QryConfiguracion.FieldValues['iFirmas'] = '2' then
       begin
          if not FileExists(global_files + global_miReporte + '_ResumendeOrdendeTrabajo2.fr3') then
            showmessage('El archivo de reporte '+global_Mireporte+'_ResumendeOrdendeTrabajo2.fr3 no existe, notifique al administrador del sistema')
          else
      ...
      if QryConfiguracion.FieldValues['iFirmas'] = '3' then
        begin
        if not FileExists(global_files + global_miReporte + '_ResumendeOrdendeTrabajo.fr3') then
             showmessage('El archivo de reporte '+global_Mireporte+'_ResumendeOrdendeTrabajo.fr3 no existe, notifique al administrador del sistema')
          else
...
begin
  try
    ...
    begin
      ...
      if QryConfiguracion.FieldValues['iFirmas'] = '2' then
      if not FileExists(global_files + global_miReporte + '_IsometricosxGenerador.fr3') then
            showmessage('El archivo de reporte '+global_Mireporte+'_IsometricosxGenerador.fr3 no existe, notifique al administrador del sistema')
          else
            frIsometricos.LoadFromFile(global_files + global_miReporte + '_IsometricosxGenerador.fr3');

      if QryConfiguracion.FieldValues['iFirmas'] = '3' then
          if not FileExists(global_files + global_miReporte + '_IsometricosxGenerador3.fr3') then
            showmessage('El archivo de reporte '+global_Mireporte+'_IsometricosxGenerador3.fr3 no existe, notifique al administrador del sistema')
          else
...
procedure TfrmEstimaInstalado.btnPlataformasClick(Sender: TObject);
begin
    if not MostrarVentana('frmEstimaciones') Then
   begin
...
procedure TfrmEstimaInstalado.btn1Click(Sender: TObject);
begin
  if not MostrarVentana('frmOrdenesCtrlObra') Then
  begin
...
begin
  ...
  begin
    ...
    if sParamTipo = 'MN' then
        if not FileExists(global_files + global_miReporte + '_ResumenMensualGeneradorMN.fr3') then
           showmessage('El archivo de reporte '+global_Mireporte+'_ResumenMensualGeneradorMN.fr3 no existe, notifique al administrador del sistema')
        else
           frxReport1.LoadFromFile(global_files + global_miReporte + '_ResumenMensualGeneradorMN.fr3');

    if sParamTipo = 'DLL' then
      if not FileExists(global_files + global_miReporte + '_ResumenMensualGeneradorDLL.fr3') then
           showmessage('El archivo de reporte '+global_Mireporte+'_ResumenMensualGeneradorDLL.fr3 no existe, notifique al administrador del sistema')
        else
