...
type
  TfrmActividadesAnexoPU = class(TForm)
    ...
    procedure Grid_ExplosionDblClick(Sender: TObject);
    procedure Grid_ExplosionGetCellParams(Sender: TObject; Field: TField;
      AFont: TFont; var Background: TColor; Highlight: Boolean);
    procedure ExplosionInsumos1Click(Sender: TObject);
...
begin
  ...
  if connection.configuracion.FieldValues['sExplosion'] = 'Recursos por Concepto/Partida' then
    ...
    //Incializacion de vector.
  Ocultar[1,1] := 'PERSONAL';
  Ocultar[2,1] := 'EQUIPO';
  Ocultar[3,1] := 'HERRAMIENTAS';
  Ocultar[4,1] := 'MATERIAL';
  Ocultar[5,1] := 'BASICOS';
  Ocultar[1,2] := '+';
  Ocultar[2,2] := '+';
  Ocultar[3,2] := '+';
...
procedure TfrmActividadesAnexoPU.ActividadesxAnexoCalcFields(DataSet: TDataSet);
begin
...
begin
  ...
  if not FileExists(global_files + global_miReporte + '_ficha_tecnica.fr3') then
     showmessage('El archivo de reporte '+global_Mireporte+'_ficha_tecnica.fr3 no existe, notifique al administrador del sistema')
  else
     AnexoC.ShowReport;  //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
end;
...
begin
  ...
  Application.CreateForm(TFrmParamsFichaTecnica,FrmParamsFichaTecnica);
  try
    FrmParamsFichaTecnica.swbs:=ActividadesxAnexo.FieldValues['sWbs'];
    FrmParamsFichaTecnica.snumeroActividad:=ActividadesxAnexo.FieldValues['sNumeroActividad'];
    FrmParamsFichaTecnica.ShowModal;
  finally
    FrmParamsFichaTecnica.Destroy;
  end;
...
procedure TfrmActividadesAnexoPU.Grid_ExplosionDblClick(Sender: TObject);
begin
  if Grid_Explosion.Fields[0].Value = '+' then
  begin
       if Grid_Explosion.Fields[9].Value = 'PERSONAL' then
          Ocultar[1,2] := '-';
       if Grid_Explosion.Fields[9].Value = 'EQUIPO' then
          Ocultar[2,2] := '-';
       if Grid_Explosion.Fields[9].Value = 'HERRAMIENTAS' then
          Ocultar[3,2] := '-';
       if Grid_Explosion.Fields[9].Value = 'MATERIAL' then
          Ocultar[4,2] := '-';
       if Grid_Explosion.Fields[9].Value = 'BASICOS' then
          Ocultar[5,2] := '-';
       Explosion;
  end;
...
procedure TfrmActividadesAnexoPU.Grid_ExplosionGetCellParams(Sender: TObject;
  Field: TField; AFont: TFont; var Background: TColor; Highlight: Boolean);
begin
If (Sender as TrxDBGrid).DataSource.DataSet.State = dsBrowse Then
        If rxExplosion.RecordCount > 0 Then
        Begin
            AFont.Color := clNavy;
            If ((Sender as TrxDBGrid).DataSource.DataSet.FieldByName('Espacio').AsString = '+') or ((Sender as TrxDBGrid).DataSource.DataSet.FieldByName('Espacio').AsString = '-')then
            begin
                AFont.Color := clBlack;
                Afont.Style := [fsBold];
                Afont.Size := Afont.Size + 1 ;
                Background := EsBkColor[(Sender as TrxDBGrid).DataSource.DataSet.FieldByName('Nivel').AsInteger + 1];
            end;
            If ((Sender as TrxDBGrid).DataSource.DataSet.FieldByName('Nivel').AsInteger = 0) then
            begin
                AFont.Color := clBlue;
                Afont.Style := [fsBold];
                Afont.Size := Afont.Size + 1 ;
...
begin
  ...
  Excel.Range['A5:G' + IntToStr(iFila + 5)].Select;
  DarFormato(False, 'izq', False, True, 10);
  Excel.Selection.Font.Size := 10;

  Excel.Range['A5:G5'].Select;
  Excel.Selection.Font.Color := 2;
  Excel.Selection.Font.Bold := True;
  Excel.Selection.Interior.ColorIndex := 46;
  ...
  begin
      ...
      query1.SQL.Add('select i.sIdInsumo, i.mDescripcion, i.sMedida, sum(r.dCantidad)as cantidad, '+
                     ...
                     'inner join insumos i on(i.sContrato = :codigo and r.sIdInsumo = i.sIdInsumo) '+
      ...
      query1.ParamByName('codigo').AsString           := connection.contrato.FieldByName('sCodigo').AsString;
      query1.Open;
      ...
      begin
        while not query1.Eof do begin
          ...
          Excel.Range['G' + IntToStr(iFila) + ':G' + IntToStr(iFila)].Select;
          Excel.Selection.Formula := '=E' + IntToStr(iFila) + '* F' + IntToStr(iFila);
          Excel.Selection.NumberFormat := '#,##0.00';
          EstablecerContornos;
  ...
  Excel.Range['G' + IntToStr(iFila) + ':G' + IntToStr(iFila)].Select;
  Excel.Selection.Formula := '=SUM(G6:G' + IntToStr(iFila - 1);
  Excel.Selection.NumberFormat := '$' + '#,##0.00';
  Excel.Selection.Font.Size := 10;
  EstablecerContornos;
...
begin
    ...
    begin
         ...
         begin
                  ...
                  if tipo[x]= 'PERSONAL' then
                     linea  := ' and ra.sIdPersonal <> ra.snumeropaquete ';

                  if tipo[x]= 'HERRAMIENTAS' then
                     linea  := ' and ra.sIdHerramientas <> ra.snumeropaquete ';
                  ...
                  if TipoExplosion = 'recursosanexo' then
                     sentencia := 'select ra.'+folio[x]+' as Id, i.'+Descripcion+' as Descripcion, i.sMedida, SUM(ra.dCantidad) as dCantidad, ra.dCostoMN, ra.dCostoDLL  from '+tablas[x]+' ra  '+
                                  'LEFT join '+catalogo[x]+' i  '+
                                  'on ((:principal=-1 or (:principal<>-1 and i.sContrato = ra.sContrato)) and i.'+
                                  folio[x]+' = ra.'+folio[x]+' '+almacen+' ) '+
                                  'where  ra.sContrato =:Contrato '+
                                  linea+' group by ra.'+folio[x]+ '' ;
 ...
 procedure TfrmActividadesAnexoPU.ExplosionInsumos1Click(Sender: TObject);
begin
 if rxExplosion.RecordCount = 0 then
  begin
    messageDLG('Para Imprimir Haga clic sobre Explosion de Insumos.', mtInformation, [mbOk], 0);
   ...
   if not FileExists(global_files + 'explosion_insumos.fr3') then
       showmessage('El archivo de reporte ' + 'explosion_insumos.fr3 no existe, notifique al administrador del sistema')
   else
     begin
      AnexoC.LoadFromFile(global_files + 'explosion_insumos.fr3');
      AnexoC.ShowReport;  //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP)); ;
     end;
end;
