...
type
  TfrmConsumibles = class(TForm)
    ...
    PreciossDescripcion: TWideStringField;
    PreciosdIdFecha: TDateField;
    PreciossContrato: TWideStringField;
    PreciossNumeroActividad: TWideStringField;
    PreciosdPrecios: TFloatField;
    PreciossIdGrupo: TWideStringField;
    ...
    insumossContrato: TWideStringField;
    insumossIdInsumo: TWideStringField;
    insumossIdProveedor: TWideStringField;
    insumossIdAlmacen: TWideStringField;
    insumossTipoActividad: TWideStringField;
    ...
    insumossModelo: TWideStringField;
    insumosiIdSubfamilia: TIntegerField;
    insumossIdMarca: TWideStringField;
    ...
    qryMarcassIdMarca: TWideStringField;
    qryMarcassMarca: TWideStringField;
    qryTipoSubFamiliaiIdTipoSubFamilia: TIntegerField;
    qryTipoSubFamiliaiIdSubFamilia: TIntegerField;
    qryTipoSubFamiliasTipo: TWideStringField;
    insumosiIdTipoSubFamilia: TIntegerField;
    insumosdTalla: TWideStringField;
    MainMenu1: TMainMenu;
    zkardex: TZQuery;
    Panel: tNewGroupBox;
    JvLabel1: TJvLabel;
    ...
    contra: TEdit;
    insumossEstado: TWideStringField;
    grupossIdFamilia: TWideStringField;
    grupossDescripcion: TWideStringField;
    qrySubfamiliaiIdSubfamilia: TIntegerField;
    qrySubfamiliasIdFamilia: TStringField;
    qrySubfamiliasDescripcion: TStringField;
    insumossIdGrupo: TWideStringField;
    ...
    mniImportarExistencia: TMenuItem;
    ...
    procedure mniImportarExistenciaClick(Sender: TObject);
  ...
  private
    ...
    procedure ImportarExistencias(sArchivo:TFileName;ParamAlmacen:string);
  public
    { Public declarations }
    sParam_familia,
...
procedure TfrmConsumibles.ImportarExistencias(sArchivo:TFileName;ParamAlmacen:string);
const
  Secuencia='MATERIAL';
...
var
  Cursor:TCursor;
  Excel, Libro, Hoja: Variant;
  continuar,salir,encontrado,error:Boolean;
  j,reng,x:Integer;
  Dato:String;
...
begin
  ...
  try
    ...
    if continuar then
    begin
      QMaterial.Connection:=connection.zConnection;
      QMaterial.SQL.Text:='select * from insumos where sContrato=:Contrato and sIdAlmacen=:Almacen and sIdInsumo=:Insumo';
      QMaterial.ParamByName('contrato').AsString:=global_contrato;
      QMaterial.ParamByName('Almacen').AsString:=ParamAlmacen;
      ...
      try
        try
          Hoja := excel.WorkBooks[excel.WorkBooks.Count].WorkSheets[1];
          Hoja.select;

          salir:=false;
          j:=3;
          reng:=0;
          while (j<50) and (not salir) do
          begin
            encontrado:=false;
            x:=1;
            while (x<50) and (not encontrado) do
            begin
              dato:=hoja.Cells[j, x].value;
              if pos(Secuencia,uppercase(dato))>0 then
              begin
                encontrado:=true;
                salir:=true;
                reng:=j;
                columnas.Add(Secuencia+ '=' + inttostr(x));
              end;
              inc(x);
            end;
            inc(j);
          ...
          begin
            for j := 1 to 100 do
            begin
              dato:=hoja.cells[reng,j].value;
              if pos(existencia,uppercase(dato))>0 then
                columnas.Add(existencia + '=' + inttostr(j));
            end;

            inc(reng);
            continuar:=true;
            dato:=hoja.cells[reng,strtoint(columnas.Values[Secuencia])].value;
            while (dato<>'') and Continuar do
            begin
              error:=false;
              try
                dExistencia:=hoja.cells[reng,StrToInt(Columnas.Values[existencia])].value;
              except
                messagedlg('Falta La Columna ' + existencia,mterror,[mbok],0);
                Continuar:=false;
              end;
              ...
              begin
                ...
                QMaterial.ParamByName('Insumo').AsString:=Trim(Dato);
                QMaterial.Open;
                if QMaterial.RecordCount=1 then
                begin
                  QMaterial.Edit;
                  QMaterial.FieldByName('dExistencia').AsFloat:=dExistencia;
                  QMaterial.Post;
                end;
              end;
              inc(reng);
          ...
          on e:exception do
            MessageError(e.Message + #13 + #10+ e.ClassName);
        end;
      finally
        excel.Visible := true;
...
procedure TfrmConsumibles.mniImportarExistenciaClick(Sender: TObject);
begin
  if dlgOpenExcel.Execute then
  begin
    ImportarExistencias(dlgOpenExcel.FileName,Actual_almacen);
    Insumos.Refresh;
  end;
end;
