...
uses
  ...
  dxSkinXmas2008Blue, cxRadioGroup;
...
type
  TfrmReportesAlmacen = class(TForm)
    ...
    rbCompProv1: TcxRadioButton;
    zqrCompPrec: TZReadOnlyQuery;
    FrxDBPrecioProv: TfrxDBDataset;
    rbCompProvConContratos: TcxRadioButton;
    {$ENDREGION}
...
{$ENDREGION}
  ...
  {$REGION 'Comparativo Con Precio y Proveedor y Contratos'}
  if rbCompProv1.Checked or rbCompProvConContratos.Checked Then
     begin

        zqrCompPrec.Active := False ;
        zqrCompPrec.SQL.Clear ;
        zqrCompPrec.SQL.Add('select a.sContrato, a.sElaboro, a.dIdFecha, i.sIdInsumo,  a.sOrdenCompra, ap.dCosto, p.sRazonSocial, i.mDescripcion, ' +
                            'ap.dCantidad from proveedores p Inner Join anexo_pedidos a On (p.sIdProveedor=a.sIdProveedor)  ' +
                            'Inner Join anexo_ppedido ap On (ap.sContrato= a.sContrato and ap.iFolioPedido = a.iFolioPedido)  ' +
                            'Inner Join insumos i On (i.sIdInsumo=ap.sIdInsumo )') ;
      If  rbCompProv1.Checked  then
        begin
           zqrCompPrec.SQL.Add(' Where a.sContrato=:Contrato and a.dIdFecha >=:fecha1 and a.dIdFecha <=:fecha2 Order by i.sIdInsumo,  a.sOrdenCompra, dIdFecha ') ;
           zqrCompPrec.Params.ParamByName('Contrato').AsString := global_contrato ;
           zqrCompPrec.Params.ParamByName('fecha1').AsDate     := dFechaInicial; ;
           zqrCompPrec.Params.ParamByName('fecha2').AsDate     := dFechaFinal ;
           zqrCompPrec.Open ;
        end ;
       if rbCompProvConContratos.Checked Then
         begin
           zqrCompPrec.SQL.Add(' Where a.dIdFecha >=:fecha1 and a.dIdFecha <=:fecha2 Order by i.sIdInsumo,  a.sOrdenCompra, dIdFecha ') ;
           zqrCompPrec.Params.ParamByName('fecha1').AsDate     := dFechaInicial; ;
           zqrCompPrec.Params.ParamByName('fecha2').AsDate     := dFechaFinal ;
           zqrCompPrec.Open ;
         end;
          ...
          frxEntrada.PreviewOptions.ShowCaptions  := False;
          frxEntrada.Previewoptions.ZoomMode      := zmPageWidth;
          frxEntrada.LoadFromFile(global_files + global_miReporte + '_ALMCompConPrecProv.fr3');
           if not FileExists(global_files + global_miReporte + '_ALMCompConPrecProv.fr3') then
             showmessage('_El archivo de reporte '+global_miReporte+'_ALMCompConPrecProv.fr3.fr3 no existe, notifique al administrador del sistema');
...
//////////////////////////////////// SEGUIMINTO DE ORDENES DE COMPRA INTELCODE 2015 {DSAI} //////////////////
  ...
  var
    ...
    begin
      ...
      connection.QryBusca2.SQL.Add( 'select f.* from familias f '+
                                    'inner join insumos i on(f.sIdFamilia=i.sIdGrupo) '+
                                    'inner join anexo_ppedido ocp ' +
                                    'on ((:Principal=-1 or (:Principal<>-1 and i.sContrato = ocp.sContrato)) and i.sIdAlmacen = ocp.sIdAlmacen '+
                                    'and i.sIdInsumo = ocp.sIdInsumo) ' +
                                    'inner join anexo_pedidos oc ' +
                                    'on (ocp.sContrato = oc.sContrato and ocp.iFolioPedido = oc.iFolioPedido) ' +
                                    'where oc.sContrato = :Contrato and oc.dIdFecha >=:FechaI and oc.dIdFecha <=:FechaF '+
                                    'group by f.sIdFamilia ');
      ...
      If Connection.configuracion.fieldValues['sAlmcon'] = 'CONTRATOS' Then
        connection.QryBusca2.ParamByName('Principal').AsString  := 'No'
      else
        connection.QryBusca2.ParamByName('Principal').AsInteger :=-1;

      connection.QryBusca2.ParamByName('FechaI').AsDate := FechaInicio.Date;
      connection.QryBusca2.ParamByName('FechaF').AsDate := FechaFinal.Date;
